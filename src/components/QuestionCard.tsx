...
export function QuestionCard({ question, strings, value, onChange, lang }: Props) {
  return (
    <Card className="w-full px-8 py-10 sm:px-12 sm:py-14" lang={lang}>
      ...
      {question.type === "yesno" && (
        <div className="mx-auto grid max-w-sm grid-cols-1 gap-3 sm:grid-cols-3">
          {[
            { v: "yes", label: strings.yes },
            { v: "no", label: strings.no },
            { v: "unsure", label: strings.unsure },
          ]
            // The anchor question uses all three; other yes/no questions only need two.
            .filter((opt) => (question.id === "anchor" ? true : opt.v !== "unsure"))
            .map((opt) => (
...
function OptionPill({
  ...
  return (
    <motion.button
      ...
      whileTap={{ scale: 0.97 }}
      className={cn(
        "rounded-2xl border px-5 py-4 text-left font-body text-sm font-medium transition-colors sm:text-base",
        active
